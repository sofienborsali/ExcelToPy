...
class LibraryApp(tk.Tk):
    ...
    def open_edit_window(self):
        ...
        def save_changes():
           self.save_edit_table(editable_tree)
    ...
    def save_edit_table(self, editable_tree):
        updated_data = []
        for item in editable_tree.get_children():
    ...
    def update_data_from_table(self, editable_tree, data):
        updated_data = []
        for item in editable_tree.get_children():
            updated_data.append(editable_tree.item(item)["values"][1:])  # Skip the selection
        self.data.update(pd.DataFrame(updated_data, columns = data.columns))
